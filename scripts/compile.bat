...
REM Make directories if they don't exist.  These two cover all the important ones
...
set "npmVersion=%npmVersionBase:~-9,-2%"
REM This looks like "6.14.10", we need to split it on the .'s
FOR /F "tokens=1,2,3 delims=. USEBACKQ" %%A IN ('%npmVersion%') DO (
	set "npmV1=%%A"
	set "npmV2=%%B"
	set "npmV3=%%C"
)
...
REM remove old ArchHUD.lua in LuaC directory if it exists
...
REM delete old modules folder in LuaC output if it exists
rmdir /S /Q "%LuaCInputDir%autoconf" 1>NUL 2>NUL
REM setup file structure for LuaC to detect them, which should match the game's
